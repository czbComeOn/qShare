...
(function($){
	$.fn.SinaEmotion = function(target){
		...
		$(this).click(function(event){
			...
			$('#emotions').html('<div style="float:right"><a href="javascript:void(0);" id="prev">&laquo;</a><a href="javascript:void(0);" id="next">&raquo;</a></div><div class="categorys"></div><div class="emotion-container"></div><div class="page"></div>');
		...
		function showEmotions(){
			...
			$('#emotions .emotion-container').html('');
			$('#emotions .page').html('');
			cat_current = category;
			for(var i = page * 40; i < (page + 1) * 40 && i < emotions[category].length; ++i){
				$('#emotions .emotion-container').append($('<a href="javascript:void(0);" title="' + emotions[category][i].name + '"><img src="' + emotions[category][i].icon + '" alt="' + emotions[category][i].name + '" width="22" height="22" /></a>'));
			}
			$('#emotions .emotion-container a').click(function(){
				target.insertText($(this).attr('title'));
				$('#emotions').remove();
			});
			for(var i = 1; i < emotions[category].length / 40 + 1; ++i){
